...
To interact securely with Supabase, the application requires the Firebase Third-Party Auth integration to be enabled on the Supabase project. This allows Supabase to verify the Firebase token natively and securely resolve the user's identity across Row Level Security (RLS) policies and RPCs.
