...
const Play: React.FC = () => {
  ...
  const { currentRoom, players: playersList, loading: contextLoading } = useMultiplayer();
  ...
  useEffect(() => {
    ...
    const chatRef = dbQuery(dbRef(db, `rooms/${currentRoom.id}/chat`), dbLimitToLast(50));
  ...
  useEffect(() => {
      if (currentRoom?.gameState?.startTime && (currentRoom.gameState as any).timerDuration) {
          ...
          const update = () => {
              ...
              if (remaining > 0) animationFrameRef.current = requestAnimationFrame(update);
          };
          update();
      }
  ...
  if (!currentRoom) return <div className="min-h-screen flex items-center justify-center bg-slate-900 text-emerald-500">Connecting...</div>;
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans text-white transition-colors duration-1000 ${streak > 25 ? 'bg-[#1a0505]' : 'bg-[#050914]'}`}>
      ...
      <div className={getPlayerListClasses()}>
         ...
         <div className="flex-1 overflow-y-auto">
            {playersList.map((p) => (
              <div key={p.id} className={`grid grid-cols-[1fr_50px_40px] px-3 py-3 text-xs items-center transition-colors border-b border-slate-800/50 ${p.id === user?.uid ? 'bg-emerald-900/20' : 'hover:bg-white/5'}`}>
                  <div className="flex items-center gap-2 overflow-hidden">
                     {/* Placeholder Avatar */}
                     <div className={`w-2 h-2 rounded-full ${p.status === 'alive' || p.status === 'connected' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                     <span className={`truncate font-medium ${p.id === user?.uid ? 'text-emerald-400' : 'text-white'} ${p.status === 'eliminated' ? 'line-through text-slate-500' : ''}`}>
                       {p.name}
                     </span>
                  </div>
                  <div className="text-center font-mono text-emerald-400">{p.score || 0}</div>
                  <div className="text-right font-mono text-yellow-400">{p.wins || 0}</div>
              </div>
            ))}
         </div>
      </div>
        ...
        <div className="w-full max-w-lg mb-8 sm:mb-12">
           ...
           <form onSubmit={handleSubmit}>
              <input
                ...
                disabled={!isInputEnabled || status !== 'playing' || myStatus === 'eliminated'}
                placeholder={myStatus === 'eliminated' ? "ELIMINATED" : "Type word..."}
