  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0f172a] p-4 relative overflow-hidden">
      ...
      <div className="z-10 text-center flex flex-col items-center">
        ...
        <Link 
          to={user ? "/lobby" : "/auth"} 
          className="px-10 py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 rounded-full font-bold text-lg transition-all transform hover:scale-105 shadow-lg shadow-emerald-500/20 tracking-wide"
        >
      ...
      {!user && (
        <div className="absolute bottom-8 text-slate-500 text-sm">
          Already have an account? <Link to="/auth" className="text-emerald-400 hover:underline">Log in</Link>
        </div>
      )}
    </div>
  );
